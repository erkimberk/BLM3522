            ...
            container.innerHTML = data.alerts
                ...
                <div class="alert alert-${alert.severity}">
                    ...
                    <div class="alert-details">
                        ...
                        <span>Değer: ${alert.value ? alert.value.toFixed(2) : "N/A"}</span>
                        <span>Eşik: ${alert.threshold || "N/A"}</span>
                    </div>
                </div>
            `
                )
...
function updateSensorCard(sensorType) {
    ...
    if (!latest) {
        // Veri yok - mesaj göster
        const typeMap = {
            "traffic_light": { id: "traffic", name: "🚦 Trafik" },
            "air_quality": { id: "air", name: "💨 Hava Kalitesi" },
            "trash_bin": { id: "trash", name: "🗑️ Çöp Kutusu" }
        };
        
        const type = typeMap[sensorType];
        if (type) {
            document.getElementById(`${type.id}-value`).textContent = "--";
            document.getElementById(`${type.id}-status`).textContent = "📭 Veri Yok";
        }
        return;
    }
